		...
		<section class="content">
			<div class="container-fluid">
				<?php if (isset($_GET['type']) && $_GET['type'] == 'nam_hoc') { ?>
					...
					?>
					<a class="btn btn-primary" href="namhoc.php">
						<- Quay lại</a>
							<div class="row mt-5">
								<div class="col-12">
									<div class="container-fluid">
										<form action="<?php echo $_GET['action'] == 'them' ?  'process_add_nam_hoc.php' :  'process_edit_nam
										_hoc.php' ?>" method="post">
											<div class="form-group">
												<?php if (isset($_GET['type']) &&  $_GET['type'] == "nam_hoc" &&  $_GET['action'] == 'sua') { ?>
													<input name="maNH" type="hidden" value="<?= $nam_hoc_sua_each['maNH'] ?>">
												<?php } ?>
												<label for="namHoc">Năm học</label>
												<input type="text" class="form-control" name="namHoc" placeholder="Ví dụ 2020 - 2021" value="<?php if (isset($_GET['type']) &&  $_GET['type'] == "nam_hoc" &&  $_GET['action'] == 'sua') {
																																					echo $nam_hoc_sua_each['namHoc'];
																																				} ?>" />
											</div>
											<div class=" form-group">
												<button type="submit" class="btn btn-info float-right" id="NamHocAddSubmit"><?php echo $_GET['action'] == 'them' ?  'Thêm' :  'Sửa' ?> thông tin</button>
											</div>
										</form>
									</div>
								</div>
																		...
																		<tbody>
																			<?php foreach ($nam_hocs as $nam_hoc) : ?>
																				<tr role="row" class="odd">
																					<td class="sorting_1"><?= $nam_hoc['maNH'] ?></td>
								...
								<div class="col-xl-4">
									<div class="card card-info">
										<div class="card-header">
											...
											<button type="button" class="btn btn-outline-success bg-light btn-xs btn-flat float-right p-1" onclick="$('#NamHocModalAdd').modal({show: true});"><i class="fas fa-plus-circle"></i><a href="?type=nam_hoc&action=them">Thêm mới</a></button>
										</div>
										<div class="card-body">
											<div id="NamHocTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
												<div class="row">
																			...
																			<?php foreach ($nam_hocs as $nam_hoc) : ?>
																				<tr role="row" class="odd">
																					...
																					<td><a class="btn btn-danger btn-sm float-right deleteable ml-2" href="process_delete_nam_hoc.php?id=<?= $nam_hoc['maNH'] ?>"><i class="fas fa-trash"></i>Xoá</a>
																						<a class="btn btn-info btn-sm float-right editable" href="?type=nam_hoc&action=sua&id_sua=<?= $nam_hoc['maNH'] ?>"><i class="fas fa-pencil-alt"></i>Sửa</a>
																					</td>
																				</tr>
																			<?php endforeach; ?>
																		</tbody>
														...
														<table class="table table-striped projects dataTable no-footer" id="KhoiLopTable" width="100%" role="grid" aria-describedby="KhoiLopTable_info" style="width: 100%;">
															...
															<tbody>
																<tr role="row" class="odd">
																	...
																	<td>10</td>
																	<td><a class="btn btn-danger btn-sm float-right deleteable ml-2" href="#"><i class="fas fa-trash"></i>Xoá</a>
											...
											<div id="HocKyTable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
												<div class="row">
													<div class="col-sm-12">
														...
														<div id="HocKyTable_processing" class="dataTables_processing card" style="display: none;">Đang xử lý...</div>
													</div>
												</div>
											</div>
										</div>
